...
import { EXPLORATION_PROMPTS } from '@/lib/wizard/explorationPrompts';
...
interface DreamExplorerProps {
  ...
  /** When true, skip the prioritize phase and complete directly — prioritize lives in the dashboard */
  skipPrioritize?: boolean;
}

export function DreamExplorer({ onComplete, initialPromptStates, onStateChange, initialPriorities, onPrioritiesChange, skipPrioritize = false }: DreamExplorerProps) {
        ...
        <div className="flex items-center gap-2">
          ...
          {totalAnswered >= 1 && (
            <Button
              variant="outline"
              size="sm"
              onClick={skipPrioritize ? handleFinish : enterPrioritize}
              className="text-xs"
            >
          ...
          {isLast && (
            <Button
              variant="default"
              size="sm"
              onClick={(!skipPrioritize && totalAnswered > 0) ? enterPrioritize : handleFinish}
              className="text-xs"
              data-testid="explorer-complete"
            >
              {(!skipPrioritize && totalAnswered > 0) ? `Prioritize ${totalAnswered} answer${totalAnswered === 1 ? '' : 's'} ✨` : 'Finish'}
            </Button>
          )}
        </div>
      </div>
